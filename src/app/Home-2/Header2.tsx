      ...
      <div className="mx-auto flex h-[88px] w-full max-w-[1320px] items-center justify-between px-7 lg:px-12">
        ...
        >
          <svg width="22" height="12" viewBox="0 0 22 12" aria-hidden>
            {open ? (
              <path d="M2 1l18 10M20 1L2 11" stroke="currentColor" strokeWidth="1.5" />
            ) : (
          ...
          <nav className="flex flex-col pt-6" aria-label="Mobile">
            {NAV.map((n) => (
              <Link
                key={n.href}
                href={n.href}
                className="flex items-center justify-between border-b border-line py-5 font-display text-2xl text-navy"
              >
                {n.label}
